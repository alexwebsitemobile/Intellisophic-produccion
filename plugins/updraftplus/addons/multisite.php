	...
	class UpdraftPlus_Options {
		...
		public static function mass_options_update($new_options) {
			...
			foreach ($new_options as $key => $value) {
				...
				} elseif ('updraft_updraftvault' !== $key && preg_match('/^updraft_(.*)$/', $key, $matches) && in_array($matches[1], $services_to_register) && is_callable(array($updraftplus, $matches[1].'_options_filter'))) {
					$options[$key] = call_user_func(array($updraftplus, $matches[1].'_options_filter'), $value);
